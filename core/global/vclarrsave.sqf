...
while {true} do {
   for "_i" from 0 to (count INV_PLAYERSTRINGLIST -1) do {
      INV_PLAYERLIST SET [_i, call compile (INV_PLAYERSTRINGLIST select _i)]
   };

   for "_i" from 0 to (count INV_VehicleArray -1) do {
      if (isNull (INV_VehicleArray select _i)) then {
         INV_VehicleArray set [_i, ""];INV_VehicleArray = INV_VehicleArray - [""];
      };
   };	
   ...
   for "_i" from 0 to (count INV_ServerVclArray -1) do { 
      if (isNull (INV_ServerVclArray select _i)) then {
         INV_ServerVclArray set [_i, ""];INV_ServerVclArray = INV_ServerVclArray - [""];
      };
   };
