...
//===================================FARMING===============================================
   for [{_i = 0}, {_i < (count INV_FarmItemArray)}, {_i = _i + 1}] do {
      ...
      if ((_isInArea) and (_hasVehicle) and (speed (vehicle player) > 2 or ((_arr select 4) select 0) == "Ship")) then {
         [(_arr select 1), (_arr select 2), (_arr select 3), (_arr select 4)] execVM "core\client\gathergen.sqf";
      };
   };

/* TODO: Missing Factories completed. Maybe Implement.
//=======================================FACTORIES================================================
   for [{_i=0}, {_i < (count INV_ItemFabriken)}, {_i=_i+1}] do {
      ...
      if (player distance _flag <= 5) then {
         if ( (_owner) and (_a2 == 0) ) then {
            INV_action_facablage     = player addAction [localize "STRS_inv_fac_storage", "core\client\storage.sqf", [_ablage, "save"]];
...
//======================================LICENSES=========================================
   for [{_i = 0}, {_i <= (count INV_Lizenzen)}, {_i = _i + 1}] do {
      ...
      //TODO: _flag is being undefined. This has to be due to INV_Lizenzen with an undefined object.
      if (!isNil "_flag") then {
         if ((player distance _flag <= 5) and !(_license call INV_HasLicense) and (_added == 0)) then {
            call compile format ["a_license%1 = player addAction [format[localize ""STRS_inv_actions_buy"", ""%2"", %3], ""core\client\addlicense.sqf"", [%1, ""add""]];", _i, _licensename, (_cost call ISSE_str_IntToStr)];
            _Arr2 set [_i, 1];
         };
