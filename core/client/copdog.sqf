...
player groupChat format["%1 bought a dog for %2", rolestring, (dog_cost call ISSE_str_IntToStr)];																						
	...
	If (iscop) Then
	{
		...
		call compile format ["_type createUnit [[(getPos player select 0) + (1 * sin (getDir player)), (getPos player select 1) + (0.3 * cos (getDir player)), 0], _grp, '%1 = this;this addEventHandler [""killed"", {[(_this select 1), %1] execVM ""core\client\dogkilled.sqf"";}];']", _name];
		...
		[
			...
			["", [], "", -1, [], "0", "0"],
			["Attack Target", [5], "", -5, [["expression", "nul = [cursorTarget] execVM 'core\client\dogAttack.sqf'"]], "1", "1", "\ca\ui\data\cursor_attack_ca.paa"],
			["Drug Search", [6], "", -5, [["expression", "nul = [cursorTarget] execVM 'core\client\dogsearch.sqf'"]], "1", "1", "\ca\ui\data\cursor_attack_ca.paa"],
			//["Drug Search", [6], "", -5, [["expression", "nul = [cursorTarget] execVM 'core\client\dogSearch.sqf'"]], "1", "1", "\ca\ui\data\cursor_igui_scroll_ca.paa"],
			["Search EOD", [7], "", -5, [["expression", "nul = [] execVM 'core\client\dogSearch1.sqf'"]], "1", "1", "\ca\ui\data\cursor_igui_scroll_ca.paa"],
			["", [], "", -1, [], "0", "0"],
			["Board Car", [8], "", -5, [["expression", "nul = [1, cursorTarget] execVM 'core\client\dogVehicle.sqf'"]], "1", "CursorOnEmptyVehicle", "\ca\ui\data\cursor_getin_ca.paa"],
			["Dismount", [9], "", -5, [["expression", "nul = [2] execVM 'core\client\dogVehicle.sqf'"]], "1", "0", "\ca\ui\data\icon_board_out_ca.paa"],
