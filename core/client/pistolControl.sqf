   ...
   [] spawn {
      ...
      while { true } do {
         ...
         if (_prevention && !(saveWeaponPistol select 1)) then {
            saveWeaponPistol set [2,true]; player removeAction holsterPistol; player removeAction unholsterPistol;
         } else { 
            if (saveWeaponPistol select 2) then {
               holsterPistol = player addAction ["Holster Pistol","core\client\pistolControl.sqf",[true]]; saveWeaponPistol set [2,false];
            };
         };
...
if (!_holster) then {
	...
	holsterPistol = player addAction ["Holster Pistol","core\client\pistolControl.sqf",[true]];
	...
	unholsterPistol = player addAction ["Unholster Pistol","core\client\pistolControl.sqf",[false]];
...
waitUntil {!isNil "saveWeaponPistol"};
