...
switch _key do {
   ...
   //L key
	...
	case 20: {
      ...
      if(!isnull _vcl)then{call compile format['[0,0,0,["%3", "public", ["vcl", "%2", %1]]] execVM "core\client\storage.sqf";', _vcl, (typeOf _vcl), format["%1_storage", _vcl]];}; 
	};
	...
	case 18: {
      ...
      for [{_i=1}, {_i < 3}, {_i=_i+1}] do {
         ...
         if (iscop and !(isnull _civ) and _civ in civarray) exitwith {
            _i = 4;
            call compile format['[0,0,0, ["civmenu", "%1", %1]] execVM "core\client\interact.sqf";', _civ];
            _handled=true;
         };
         
         if (!(isnull _civ) and _civ in shopusearray) exitwith {
            _i = 4;
            if(iscop and _civ in drugsellarray)exitwith{_civ execVM "core\client\drugsearch.sqf"};
	...
	case 41: {
      ...
      if(iscop) then {
         if(vehicle player == player)then{[0,0,0,["copmenulite"]] execVM "core\client\maindialogs.sqf"};
         if(vehicle player != player)then{[0,0,0,["copmenu"]] execVM "core\client\maindialogs.sqf"};
         _handled=true;
		};
	};
	...
	case 6: {
      ...
      ["use", "spikestrip"] execvm "core\global\createobject.sqf";
		_handled=true;
	};
	...
	case 5: {
      ...
      [0,1,2,["coplog"]] execVM "core\client\maindialogs.sqf";
      _handled=true;
   };
