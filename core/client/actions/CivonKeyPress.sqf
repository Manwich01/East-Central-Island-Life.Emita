...
switch _key do {
	...
	case 21: {
      ...
      _handled = true;
      [] execVM "core\client\animdlgopen.sqf";
	};
	...
	case 15: {
      if (INV_shortcuts) then {
         INV_shortcuts=false;
         titletext["ECIL keys off", "PLAIN DOWN"];
         [] execVM "core\client\actions\actionsRemove.sqs";
      } else {
         INV_shortcuts=true;
         titletext["ECIL keys on", "PLAIN DOWN"];
   ...
   case 38: {
      ...
      ["schluessel", _vcl, 0] execVM "core\global\keys.sqf";
      _handled=true;
   };
	...
	case 20: {
      ...
      _vcl = _vcls select 0;
      if (player != driver _vcl) exitwith {
         player groupchat "You must be in the drivers seat to get to the trunk";
      };
      
      if (!(_vcl in INV_VehicleArray) and typeof _vcl == "TKOrdnanceBox_EP1") exitwith {
         player groupchat "You do not have the keys to this hideout.";
      };
      ...
      if (!isnull _vcl) then {
         call compile format['[0,0,0,["%3", "public", ["vcl", "%2", %1]]] execVM "core\client\storage.sqf";', _vcl, (typeOf _vcl), format["%1_storage", _vcl]];
      };
	};
	...
	case 18: {
      ...
      for [{_i=1}, {_i < 3}, {_i=_i+1}] do {
         ...
         if (isciv and !(isnull _civ) and _civ in playerarray) exitwith {
            _i = 4;
            call compile format['[0,0,0, ["civinteraktion", "%1", %1]] execVM "core\client\interact.sqf";', _civ];
            _handled=true;
			};

         if (!(isnull _civ) and _civ in shopusearray) exitwith {
            _i = 4;
            if (iscop and _civ in drugsellarray) exitwith {
               _civ execVM "core\client\drugsearch.sqf"
            };
            
            _id = _civ call INV_getshopnum;
            [0,0,0,[_id]] execVM "core\client\shopdialogs.sqf";
            _handled=true; 
         };
      ...
      if(_shift and (vehicle player == player) and call INV_isArmed) then {
         ...
         _men spawn {
            ...
            if(count _this > 0) then {
               ...
               (format ["if (player == %1) then {[""hit"", %2, ""Melee"", 1] execVM ""core\client\stun.sqf""};", _civ, player]) call broadcast;
	...
	case 2: {
      ...
      [0,0,0,["spielerliste"]] execVM "core\client\maindialogs.sqf";
	};
	...
	case 3: {
      ...
      execVM "core\client\inventory.sqf";
   };
	...
	case 6: {
      if (!iscop) then {
         ...
         _handled=true;
         if(dialog)exitwith{closeDialog 0;};
         [0,0,0,["gangmenu"]] execVM "core\client\maindialogs.sqf";
      };
	};
