...
if (_art == "copmenu") then {
   ...
   _type1 = ["police_bell206","MH6J_EP1","OH58g","UH1H_TK_GUE_EP1","MH60S","HH65C","ibr_as350_civ","tcg_taurus_pb_2","tcg_taurus_uc","tcg_taurus","tcg_taurus_wopb","olrpg_sdtauruspb","olrpg_sdtaurus","il_charger_pd","ilpd_charger_black","ilpd_charger_white","jailbus","il_bearcat","patrol","patrol_pb","patrol_np","patrol_slick","sheriff","whiteuc","blackuc","whited","greyd","whited","blackd","tcg_suburban_uc_black","tcg_suburban_uc_grey","tcg_suburban_uc_white","ilpd_beat_f101","olrpg_sdcv_pool_pb","ilpd_scu_black","ilpd_scu_white","ilpd_unmarked_scu","ilpd_unmarked_sandstone","ilpd_unmarked_maroon","ilpd_unmarked_darkblue","ilpd_unmarked_black","ilpd_unmarked_grey","ilpd_unmarked_white","ilpd_slick_b40_npb","olrpg_sdcv_slick_npb","ilpd_slick_b40_PBG","olrpg_sdcv_slick_fpb","ilpd_Traffic_black","ilpd_Traffic_white","tcg_suburban_patrol","tcg_suburban_sheriff","suburban_pd_k9","suburban_pd","olrpg_sdsub6","suburban_pd_stealth","suburban_pd_black","ilpd_scu_darkblue","ilpd_scu_grey","ilpd_scu_maroon","ilpd_scu_sandstone","ilpd_Traffic_sandstone","ilpd_Traffic_maroon","ilpd_Traffic_grey","ilpd_Traffic_darkblue","il_fordcv_taxi","tahoe_pd","olrpg_sdtahoe","tahoe_uc_black","tahoe_uc_grey","tahoe_uc_1_grey","tahoe_uc_1_blue","tahoe_uc_1_green","ilpd_slick_b40_pb","ilpd_traffic_t101","olrpg_sdcv_fpb","olrpg_sdcv_pb","tahoe_uc_maroon","tahoe_uc_white","tahoe_uc_1_maroon","tahoe_uc_1_white","tahoe_uc_1_black","ilf350swat","olrpg_sdf350"];
   ...
   if (player == vehicle player) then {
      player groupChat "You need to be in a police car or at the precinct";
		...
		if (_type in _type1) then {
         ...
         player groupChat "Your car is not equipped with a police computer";
      };
	};
};
...
if (_art == "impound") then {
   ...
   buttonSetAction [2, "[lbData [1, (lbCurSel 1)],""buy""] execVM ""core\client\impound.sqf"";"];	
};
...
if (_art == "gildenverwaltung") then {
   ...
   private "_i";
   for [{_i=0}, {_i < (count gangsarray)}, {_i=_i+1}] do {
      if ((name player) in ((gangsarray select _i) select 1)) exitWith {
         _members = ((gangsarray select _i) select 1)
      };
   };
