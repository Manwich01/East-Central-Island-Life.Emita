...
if (_art == "create") then {
   ...
   if (_text == "cops") exitWith {
      player groupchat "you can't use that name"
   };
   ...
   for "_c" from 0 to (count gangsarray - 1) do {
      if (((gangsarray select _c) select 0) == _text) exitWith {
         _exitvar = true;
      };
   };
   ...
   if (_geld < gangcreatecost) exitwith {
      player groupchat "You don't have enough money to create this gang!"
   };
...
if (_art == "join") then {
   ...
   if (_count >= 5) exitWith {
      player groupchat "Only 5 people are allowed in a gang";
   };

   if (!_canjoin) exitWith {
      player groupchat "You are not able to join this gang because the leader of this gang is not currently not recruiting"
   };
...
if(_art == "leave") then {
   ...
   if (!gangmember) exitWith {
      player groupchat "you are not in a gang!"
   };
...
if(_art == "kick") then {
   ...
   if (player == _civ) exitWith {
      player groupchat "you can't kick yourself, silly!"
   };
   ...
   while {isciv} do {
      for "_c" from 0 to (count gangsarray - 1) do {
         ...
         if (count _members > 0) then {
            ...
            if (isnull _lunit and !(player in (units startgroup))) then {
      ...
      if (!gangmember and !(player in (units startgroup))) then {
         [player] joinsilent startgroup
      };
      
      /* TODO: See if this is worth having, as it spams errors
      for "_c" from 0 to (count gangareas - 1) do {
         ...
         if (isNil "_control") then {         
            if ((player distance _gangarea) < 10 && (getPos _gangarea select 2) < 0 && _control == (call INV_mygang)) then {
